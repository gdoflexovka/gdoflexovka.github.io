...
import { CHAPTERS, getLevelById } from "@/lib/levels";

export const Route = createFileRoute("/chapters")({
  head: () => ({ meta: [{ title: "Главы — GDofLexovka" }] }),
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-background text-foreground">
      ...
      <main className="relative z-10 mx-auto max-w-5xl px-6 pt-10 pb-24">
        ...
        <h1 className="mt-2 text-4xl font-bold tracking-tight sm:text-5xl">Выбери главу</h1>
        <div className="mt-10 grid gap-4 sm:grid-cols-2">
          {CHAPTERS.length === 0 && (
            <div className="col-span-2 rounded-2xl border border-dashed border-border bg-card/40 p-8 text-center">
              <p className="text-sm text-muted-foreground">Пока нет глав. Админ скоро добавит.</p>
            </div>
          )}
          {CHAPTERS.map((c, i) => (
            <Link key={c.id} to="/play/$chapter" params={{ chapter: c.id }}
              ...
              <p className="mt-2 text-sm text-muted-foreground">{c.levelIds.length} уровней</p>
